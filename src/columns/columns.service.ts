...
export class ColumnsService {
  ...
  // 특정 보드 모든 칼럼 조회 API
  async getAllColumnByBoardId(boardId: number) {
  ...
  // 특정 보드 컬럼 상세 조회 API
  async getColumnByColumnId(id: number) {
  ...
  async swapOrder(id: number, name:string, newOrder: number){
    ...
    // 바꾸려는 칼럼 찾기
    const columnToChange = await this.getColumnByColumnId(id);
    ...
    if(columnToChange && columnAtNewOrder) {
     ...
     await queryRunner.commitTransaction(); 
      ...
      await queryRunner.rollbackTransaction();
      console.log("컬럼 순서 변경 에러:", error)
      throw new InternalServerErrorException(
        "컬럼 순서 변경 중 오류가 발생했습니다.",
      );
    } finally {
